...
uint16_t funcControlledExplosion(Board& board, Player& pl1, Player& pl2)
{
	...
	{
		qDebug() << "This card cannot be placed because the board is not to it's max size\n";
		return 1;
	}
	...
	if (isolatedSpaces(board)) {
		...
		pl1 = copyPl1;
		pl2 = copyPl2;
		qDebug() << "Can't have isolated stacks/cards..\n";
		return 1;
	}
...
uint16_t funcDestruction(Board& board, Player& player)
{
	...
	{
		qDebug() << "Can't use Destruction elemental card on eter card\n";
		return 1;
	}
...
uint16_t funcFire(Board& board, Player& player1, Player& player2, uint16_t value)
{
	...
	if (cardCount < 2)
	{
		qDebug() << "Not enough cards on board with chosen value\n";
		return 1;
	}
...
uint16_t funcSpark(Board& board, Player& p)  //this function needs to be reworked
{
	...
	qDebug() << "choose where from to remove one of your cards:\n";
	std::cin >> x1 >> y1;

	qDebug() << "choose a destination to place the card\n";
	std::cin >> x2 >> y2;
	...
	{
		qDebug() << "here are the cards that you can replace\n";
		int index = 0;
		for (int16_t k = 0; k < matrix[x1][y1].size(); ++k)
		{
			if (matrix[x1][y1][k].GetColor() == p.GetPlayerColor())
			{
				...
				qDebug() << k + 1 << " .value:" << matrix[x1][y1][k] << '\n';
			}
		}
		qDebug() << "which card (index) would you like to take out and place on a different place?\n";
		int chosenIndex;
		std::cin >> chosenIndex;
		while (!(chosenIndex >= 1 && chosenIndex < matrix[x1][y1].size()))
		{
			qDebug() << "invalid index, try an index that belongs to the range\n";
			std::cin >> chosenIndex;
		}
		...
		{
			qDebug() << "Failed to place minion card\n";
			return 1;
		}
	}
	else
	{
		qDebug() << "invalid position, no cards here\n";
		return 1;
	}
...
uint16_t funcWaterfall(Board& board, uint16_t x, uint16_t y)
{
	...
	if (condition < 3)
	{
		qDebug() << "There must be at least 3 cards in this column for this elemental power card to work\n";
		return 1;
	}
	else
	{
